  ...
  methods: {
    ...
    async $_ocUpload_addDropToQue (e) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const exists = this.checkIfElementExists(items[i])
        if (items[i].isDirectory) {
          if (!exists) {
            this.processDirectoryEntryRecursively(items[i]).then(() => {
              this.$emit('success', null, items[i].name)
            })
          } else {
            this.showMessage({
              title: this.$gettextInterpolate(this.$gettext('Folder %{folder} already exists.'), { folder: items[i].name }, true),
              status: 'danger'
            })
          }
          if ((i + 1) === items.length) this.$_ocUploadInput_clean()
        } else {
          if (!exists) {
            items[i].file(file => {
              this.$_ocUpload(file, file.fullPath)
            })
          } else {
            const translated = this.$gettext('File %{file} already exists.')
            this.setOverwriteDialogTitle(this.$gettextInterpolate(translated, { file: items[i].name }, true))
            ...
            if (overwrite) {
              items[i].file(file => {
                this.$_ocUpload(file, file.fullPath, exists.etag)
              })
              if ((i + 1) === items.length) this.$_ocUploadInput_clean()
            } else if (!overwrite && (i + 1) === items.length) {
    ...
    async $_ocUpload_addFileToQue (e) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const overwrite = await this.$_ocUpload_confirmOverwrite()
        if (overwrite) {
          this.$_ocUpload(files[i], exists.etag)
          if ((i + 1) === files.length) this.$_ocUploadInput_clean()
        } else if (!overwrite && (i + 1) === files.length) {
          this.$_ocUploadInput_clean()
        }
        this.setOverwriteDialogMessage(null)
      }
    ...
    $_ocUpload_confirmOverwrite () {
      return new Promise(resolve => {
        const confirmButton = document.querySelector('#overwrite-confirm')
        const cancelButton = document.querySelector('#overwrite-cancel')
