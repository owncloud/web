...
test.describe('internal link share', () => {
  ...
  test('upload multiple small files', { tag: '@predefined-users' }, async () => {
    ...
    await ui.userUploadsSmallFilesInPersonalSpace({
      actorsEnvironment,
      filesEnvironment,
      stepUser: 'Alice',
      numberOfFiles: 50
  ...
  test('Upload large file when insufficient quota', async () => {
    ...
    await ui.userChangesQuotaOfUser({
      actorsEnvironment,
      stepUser: 'Admin',
      key: 'Alice',
      value: '0.00001'
    ...
    await ui.userTriesToUploadResources({
      actorsEnvironment,
      filesEnvironment,
      stepUser: 'Alice',
