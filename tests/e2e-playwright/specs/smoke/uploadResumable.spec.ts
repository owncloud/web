...
import { userCreatesAFileOfSize } from '../../steps/runtimeFs'
...
test.describe('Upload large resources', { tag: '@predefined-users' }, () => {
  ...
  test('Upload large resources in personal space', async () => {
    ...
    await userCreatesAFileOfSize({ fileName: 'largefile.txt', fileSize: '1GB' })
    ...
    // | resource      |
    // | largefile.txt |
    await ui.userStartsUploadingLargeResourceFromTheTempUploadDir({
      actorsEnvironment,
      filesEnvironment,
      stepUser: 'Alice',
      resource: 'largefile.txt'
    })
