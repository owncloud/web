...
test.describe('check files pagination in project space', () => {
  ...
  test('pagination', async () => {
    ...
    //   | sales    |
    //   | security |
    await api.createGroup({ usersEnvironment, stepUser: 'Admin', groupName: 'sales' })
    await api.createGroup({ usersEnvironment, stepUser: 'Admin', groupName: 'security' })
    ...
    let spaceLocator = await ui.getSpaceLocator({
      actorsEnvironment,
      stepUser: 'Brian',
      actionType: 'should not',
      space: 'team.1'
    ...
    await ui.removeAccessToMember({ actorsEnvironment, usersEnvironment, stepUser: 'Alice' })

    // Then "Alice" should not see space "team.1"
    spaceLocator = await ui.getSpaceLocator({
      actorsEnvironment,
      stepUser: 'Alice',
      actionType: 'should not',
      space: 'team.1'
