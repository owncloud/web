...
export async function userHasDeletedProjectSpace({
  ...
  const user = usersEnvironment.getUser({ key: stepUser })
  const space = spacesEnvironment.getSpace({ key: id })
  await api.graph.disableSpace({ user, space })
  await api.graph.deleteSpace({ user, space })
}

export async function addMembersToProjectSpace({
  usersEnvironment,
  stepUser,
  spaceName,
  members // Array of { shareWith, shareType, role }
}: {
  usersEnvironment: UsersEnvironment
  stepUser: string
  spaceName: string
  members: Array<{ user: string; shareType: string; role: string }>
}): Promise<void> {
