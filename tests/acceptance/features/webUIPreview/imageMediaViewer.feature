Feature: display image in media viewer on the webUI
  ...
  Scenario Outline: navigate to next and previous videos with media viewer is possible
    Given user "user1" has uploaded file "<resource-one>" to "<resource-one-name>"
    And user "user1" has uploaded file "<resource-two>" to "<resource-two-name>"
    And user "user1" has logged in using the webUI
    When the user views the file "<resource-one-name>" in the mediaviewer using the webUI
    And the user navigates to the next media resource using the webUI
    Then the file "<resource-two-name>" should be displayed in the mediaViewer
    When the user navigates to the previous media resource using the webUI
    Then the file "<resource-one-name>" should be displayed in the mediaViewer
    Examples:
      | resource-one   | resource-one-name | resource-two    | resource-two-name |
      | test_video.mp4 | test_video0.mp4   | test_video1.mp4 | test_video1.mp4   |
      | testavatar.jpg | testavatar.jpg    | testavatar.png  | testavatar.png    |
