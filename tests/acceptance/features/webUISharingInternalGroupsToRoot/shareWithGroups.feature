...
Feature: Sharing files and folders with internal groups
  ...
  Scenario Outline: share a file & folder with another internal user
    ...
    Then folder "simple-folder (2)" should be marked as shared by "Carol King" on the webUI
    And file "testimage (2).jpg" should be marked as shared by "Carol King" on the webUI
    Examples:
  ...
  Scenario: change existing expiration date of an existing share with another internal group
    ...
    Then user "Alice" should have received a share with target "lorem (2).txt" and expiration date in 7 days
    And user "Brian" should have received a share with target "lorem (2).txt" and expiration date in 7 days
  ...
  Scenario: share a resource with another internal group with default expiration date
    ...
    Then user "Carol" should have a share with these details:
      ...
      | share_with | grp1       |
      | expiration | +42        |
    And user "Alice" should have received a share with target "lorem (2).txt" and expiration date in 42 days
    And user "Brian" should have received a share with target "lorem (2).txt" and expiration date in 42 days
