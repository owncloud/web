...
#
  Scenario: conflict with a big file (when chunking is implemented this upload should be chunked)
    ...
    And as "Alice" the content of "biggg-video.mp4" in the server should be the same as the content of local file "big-video.mp4"
  ...
  Scenario: upload a new file into a sub folder
    ...
    Then no message should be displayed on the webUI
    And file "new-lorem.txt" should be listed on the webUI
    And as "Alice" the content of "simplsse-folder/new-lorem.txt" in the server should be the same as the content of local file "new-lorem.txt"
