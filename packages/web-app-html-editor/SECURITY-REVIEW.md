...
| F5/F8 | info | App relied on the deployment proxy CSP; `data:`/`blob:` exfil channels survived the opaque origin | **Fixed**: a strict iframe-scoped CSP (`default-src 'none'; form-action 'none'; ...`) is injected into the `srcdoc` (`helpers/preview.ts`) |
...
for v1 to keep the preview self-protecting and network-isolated.
