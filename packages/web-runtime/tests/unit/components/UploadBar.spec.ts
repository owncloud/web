import UploadBar from '../../../src/components/UploadBar.vue'
...
const selectors = {
  overlay: '#upload-bar',
  title: '.upload-bar-title p',
  progress: '.upload-bar-progress',
  success: '.upload-bar-success',
  error: '.upload-bar-danger',
  message: '.upload-bar-message',
  info: {
    items: '.upload-bar-items',
    item: '.upload-bar-items li'
  }
}
...
function getShallowWrapper() {
  ...
  return {
    mocks,
    wrapper: shallowMount(UploadBar, {
      global: {
