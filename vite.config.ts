        ...
        },
        {
          ...
          transformIndexHtml: {
            transform(html, { bundle }) {
              if (!bundle) {
                return ejs.render(html, { data: { WEB_APPS_MAP_POST: null } })
              }
              // get all web-app-* modules and create a map like { "web-app-foo": "js/web-app-foo-XYZ123.js", ... }
              const re = new RegExp(/js\/(web-app-.*)-.*\.(.+)/)
