...
def getRecentBuilds():
    return [{
        ...
        "commands": [
            ...
            "reference=$(awk '/Build #|Ref: refs\\\\/pull/' %s/thisBuildInfo.txt | awk -F: '{print $NF}')" % dir["web"],
            "echo $reference",
            ...
            "awk -v ref=$reference 'index($0,ref){print p} {p=$0}' %s/filteredDescriptions.txt > %s/buildsToBeStopped.txt" % (dir["web"], dir["web"]),
